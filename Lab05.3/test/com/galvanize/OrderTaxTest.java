...
public class OrderTaxTest {
  ...
  public void testTaxOnlineOrder() {
    ...
    assertEquals(20.0, order1.getTotalCost(), .0001);
    assertEquals(0.0, order1.getTax(), .0001);

    Order order2 = new Order("order-2", Location.ONLINE);
    order2.processCart(cart2);
    assertEquals(120.0, order2.getTotalCost(), .0001);
    assertEquals(0.0, order2.getTax(), .0001);
  }
  ...
  public void testTaxEuropeOrder() {
    ...
    order1.processCart(cart1);
    assertEquals(20.0, order1.getTotalCost(), .0001);
    assertEquals(3.4, order1.getTax(), .0001);

    Order order2 = new Order("order-2", Location.EUROPE);
    order2.processCart(cart2);
    assertEquals(120.0, order2.getTotalCost(), .0001);
    assertEquals(25.4, order2.getTax(), .0001);
  }
